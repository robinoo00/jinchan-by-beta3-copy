...
class Init extends Base{
    ...
    getdata(data) {
        var self = this;
        // console.log(data);
        // console.log(self.第一次加载数据);
        // console.log(self.画布上下文);
        if (self.第一次加载数据 == false) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                self.分时数据.push({ 市价: data[i]["Close"], 时间: data[i]["Time"], 开盘: Number(data[i]["Open"]), 收盘: Number(data[i]["Close"]), 最高: Number(data[i]["High"]), 最低: Number(data[i]["Low"]), 量: Number(data[i]["Volume"]), 存收: Number(data[i]["Close"]) });
            };
            self.分时数据[0]["量"] = 0;
