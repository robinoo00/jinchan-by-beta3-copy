...
class K extends React.Component {
    componentDidMount() {
        setTimeout(() => {
            ...
            draw.loading();
            draw.eve();
            const {getKDataByNum} = this.props;
            getKDataByNum();
            const {getKData} = this.props;
            k_id = setInterval(() => {
                getKData()
            },1000)
        })
            ...
            <div>
                <Flex styleName="k-nav">
                    {type_list.map((item,index) => (
                        <Flex.Item
                            style={type_choose === item.text ? {borderBottom:'1px solid #fff'} : {}}
                            styleName={"k-nav-item"} key={'k_nav_'+index}
                            onClick={this.chooseType(item).bind(this)}
                        >{item.text}</Flex.Item>
                    ))}
                </Flex>
                {/*<nav styleName="k-nav">*/}
